  ...
  useEffect(() => {
    const check = async () => {
      if (location.state?.data?.email) {
        await axiosInstance
          .get("api/newuser", {
            params: { email: location.state?.data?.email },
          })
          .then((res) => {
            if (res.data.exists) {
              ...
              updateFormData({
                ...formData,
                fname: location.state?.data?.given_name,
                lname: location.state?.data?.family_name,
    ...
    <>
      <>
        ...
        {isLoading ? (
          ...
          <div id="signupDiv" style={{ margin: "2% 15% 5% 15%" }}>
            <ConfirmDialogBox
