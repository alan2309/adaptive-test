...
function ATestScreen() {
  ...
  useEffect(() => {
    ...
    if(!localStorage.getItem("test3")){
      ...
      axiosInstance.post('api/marks/5',{
        data:{
            username:user,
            marks:total
        }
  ...
  .catch((e)=>console.log(e))
      ...
      let mm = txx.mm;
      let ss = txx.ss;
      localStorage.setItem("test3", JSON.stringify({'username':user,'STime':Date(),'FSTimer':'10','question':[],'strtTime':+ hh + ':' + mm+':'+ss,'currentQsNo':1}));
    }
    ...
    if (isMyTokenExpired) {
      ...
      if (localStorage.getItem("result")) {
        ...
        const getData = async () =>
          await axios
            .get("http://127.0.0.1:8000/api/subs/5")
            .then((res) => {
