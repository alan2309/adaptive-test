...
export default function Compiler() {
 ...
 function userInput (event){
  ...
  async function submit(e){
    e.preventDefault();
    if(customInputCheck&&user_input===undefined){alert('Please enter input')
}else{
    ...
    const response = await fetch(
      "https://judge0-ce.p.rapidapi.com/submissions",
      {
        ...
        headers: {
          ...
          "x-rapidapi-key": "d35b7822fcmshab047f684dc27bap12bb13jsnfc29849e5539", // Get yours for free at https://rapidapi.com/judge0-official/api/judge0-ce/
...
async function submitCode(e){
    ...
    //   outputText.innerHTML = `Creating Submission ... \nSubmission Created ...\nChecking Submission Status\nstatus : ${jsonGetSolution[0].status.description}`;
      if (jsonResponse[0].token&&jsonResponse[1].token&&jsonResponse[2].token) {
    ...
    console.log(jsonGetSolution)
    if(jsonGetSolution.submissions[0].status.description === "Accepted" &&
    ...
    for(var y=0;y<jsonGetSolution.submissions.length;y++){
        ...
        if (jsonGetSolution.submissions[y].stdout) {
            ...
            const compilation_error = atob(jsonGetSolution.submissions[y].compile_output);
            outputText.innerHTML = "";
            outputText.innerHTML += `\n Error :${compilation_error}`;
...
class CustomTextarea {
    ...
    initEventListeners() {
      this.textarea.addEventListener('scroll', () => {
        ...
        if (Math.abs(this.numbers.offsetHeight - this.textarea.offsetHeight - this.textarea.scrollTop) < 100) {
          this.addMoreNumbers();
        }
      });
    }
        ...
        <Row>
            <Col md={6}>
                ...
                <div className="TestHeaderComp" style={{paddingBottom:'7px'}}>
                    ...
                    <Row >
                    <div className='basicRec' style={{marginTop:'5px',height:'550px',backgroundColor:'#F7F7F7'}} >
                    <Tabs defaultActiveKey="Q1" id="uncontrolled-tab-example" style={{marginBottom:'0px !important'}}  className="mb-3">
            ...
            <Col md={6} >
                ...
                <Row style={{paddingBottom:'5px',paddingLeft:'40%',height:'40px',marginBottom:'10px'}}>
                    <Col ><button type="button" style={{color:'white',width:'fit-content'}} className='btn scTest'>Next Section</button></Col>
                    <Col ><button type="button" className='btn btn-success' style={{
                    backgroundColor: "#081466",
                    width: "fit-content",
                    borderRadius: "10px",
                    color: "white",padding:'7px 10px'
                  }}>Finish Test</button></Col>
                    
                </Row>
                <Row style={{marginTop:'5px'}}>
                    <div className="basicRec" style={{backgroundColor:'#F7F7F7',padding:'5px 5px'}}>
                    <div class='custom-textarea'>
                        <textarea class='textarea' spellCheck={false} defaultValue={inputT} onChange={(e)=>input(e)} >
                    </textarea>
                ...
                <Row style={{paddingTop:'5px',paddingBottom:'0',height:'42px'}}>
                    <Col lg={3}> 
                    ...
                    <Col lg={3}>
                    <div class="custom-control custom-checkbox" style={{paddingTop:'8px'}}>
                        <input type="checkbox" className="custom-control-input" style={{marginRight:'4px'}} onClick={(e)=>{
                            console.log(e);
                            setCustomInputCheck(e.target.checked)
                        }} id="defaultUnchecked"/>
                        <label class="custom-control-label" for="defaultUnchecked">Custom Input</label>
                    </div>
                    </Col>
                    <Col lg={3}> 
                    <button
              type="submit"
              className="btn scTest ml-2 mr-2 " style={{color:'white',width:'fit-content'}}
              onClick={submit}
            >
              <i className="fas fa-cog fa-fw"></i> Run
              </button>
                ...
                <Row>
                <div className='basicRec' style={{marginTop:'5px',height:'190px',backgroundColor:'#F7F7F7'}} >
                    <Tabs defaultActiveKey="customInput" id="uncontrolled-tab-example"  className="mb-3">
                        <Tab eventKey="customInput"  title="custom input" >
          
          <textarea id="style-4" className="scrollbar customInput" spellCheck={false} onChange={userInput}></textarea>
                        </Tab>
                        <Tab  eventKey="Result" title="result">
                        <textarea  disabled readOnly className="scrollbar codeOutput" id="style-4"
                        ></textarea>
